...
type MainClient interface {
	RegisterProvider(ctx context.Context, opts ...grpc.CallOption) (Main_RegisterProviderClient, error)
...
func (c *mainClient) RegisterProvider(ctx context.Context, opts ...grpc.CallOption) (Main_RegisterProviderClient, error) {
	stream, err := c.cc.NewStream(ctx, &Main_ServiceDesc.Streams[0], "/grpcApi.Main/RegisterProvider", opts...)
	if err != nil {
		return nil, err
	}
	x := &mainRegisterProviderClient{stream}
	return x, nil
}

type Main_RegisterProviderClient interface {
	Send(*Provider) error
	Recv() (*Empty, error)
	grpc.ClientStream
}

type mainRegisterProviderClient struct {
	grpc.ClientStream
}

func (x *mainRegisterProviderClient) Send(m *Provider) error {
	return x.ClientStream.SendMsg(m)
}

func (x *mainRegisterProviderClient) Recv() (*Empty, error) {
	m := new(Empty)
...
type MainServer interface {
	RegisterProvider(Main_RegisterProviderServer) error
...
func (UnimplementedMainServer) RegisterProvider(Main_RegisterProviderServer) error {
	return status.Errorf(codes.Unimplemented, "method RegisterProvider not implemented")
}
...
func _Main_RegisterProvider_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(MainServer).RegisterProvider(&mainRegisterProviderServer{stream})
}

type Main_RegisterProviderServer interface {
	Send(*Empty) error
	Recv() (*Provider, error)
	grpc.ServerStream
}

type mainRegisterProviderServer struct {
	grpc.ServerStream
}

func (x *mainRegisterProviderServer) Send(m *Empty) error {
	return x.ServerStream.SendMsg(m)
}

func (x *mainRegisterProviderServer) Recv() (*Provider, error) {
	m := new(Provider)
...
var Main_ServiceDesc = grpc.ServiceDesc{
	...
	Streams: []grpc.StreamDesc{
		{
			StreamName:    "RegisterProvider",
			Handler:       _Main_RegisterProvider_Handler,
			ServerStreams: true,
			ClientStreams: true,
		},
	},
